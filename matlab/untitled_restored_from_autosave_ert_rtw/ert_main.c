...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(untitled_restored_from_autos_M) == (NULL));
  while (runModel) {
    ...
    untitled_restored_from_autosave_step();

    /* Get model outputs here */

    /* Trigger External Mode event */
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(untitled_restored_from_autos_M) ==
                       (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(untitled_restored_from_autos_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  untitled_restored_from_autosave_terminate();
  extmodeReset();
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(untitled_restored_from_autos_M, 0);
  ...
  /* Initialize model */
  untitled_restored_from_autosave_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(untitled_restored_from_autos_M->extModeInfo,
    &rtmGetTFinal(untitled_restored_from_autos_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(untitled_restored_from_autos_M, true);
    }
  }
